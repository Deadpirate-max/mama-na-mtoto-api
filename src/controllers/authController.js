...
const setPin = asyncHandler(async (req, res) => {
  ...
  await pool.query(
    "UPDATE mothers SET pin_hash = $1, pin_set = TRUE WHERE phone = $2",
    [pinHash, phone],
  );
...
const verifyPin = asyncHandler(async (req, res) => {
  ...
  if (!match) {
    throw new ApiError(401, "Incorrect PIN", errorCodes.UNAUTHORIZED);
  }
...
const recoverAccount = asyncHandler(async (req, res) => {
  ...
  // Store recovery token in the response temporarily (or a secure session)
  const recoveryToken = "secure_token_placeholder"; // In a real app, generate a JWT here
...
const confirmRecovery = asyncHandler(async (req, res) => {
  ...
  // Verify OTP logic (exact same as verifyOtp)
  const { rows } = await pool.query(
    `SELECT id, code, expires_at FROM otp_codes WHERE phone = $1 AND verified = false ORDER BY created_at DESC LIMIT 1`,
    [newPhone],
  );
